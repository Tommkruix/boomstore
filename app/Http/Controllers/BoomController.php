...
class BoomController extends Controller
{
    ...
    {
    	$data = strtolower($request->get('search'));
    	$data = explode(' ', $data);
    	$data = implode('-', $data);
    	
    	return redirect('/search-result/'. $data);
    }
    public function search_result($dt)
    {
    	$id = 1;
    	if (!is_numeric($id)) {
    		return redirect('/');
    	...
    	foreach ($data as $v) {
	    	foreach ($prod as $key => $val) {
	    		$gm = explode(' ', strtolower($val->product_name));
	    		foreach ($gm as $ke) {
	    			if ($ke == $v) {
	    				$items[] = $val->id;
	    			}
    	...
    	return view('sr')->with(compact(['getc','tags','cats','id','items']));
    }
}
